...
export default function DashboardPage() {
  ...
  const displayName = userProfile?.username || userProfile?.id?.split('@')[0] || 'Member';
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${
                                bot.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-zinc-400'
                              }`}
                            />
                            <span>
                              {bot.status === 'ACTIVE'
                ...
                {subBots.map((bot) => (
                  <div key={bot.id} className="p-4 space-y-2.5">
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span>Prefix: <code className="px-1.5 py-0.5 rounded bg-muted font-mono">{bot.customPrefix || '.'}</code></span>
            ...
            <div className="space-y-1">
              ...
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, ((userProfile?.creditScore ?? 500) / 1000) * 100)}%` }}
                />
              </div>
            </div>
          </div>
