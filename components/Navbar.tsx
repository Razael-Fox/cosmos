    ...
    return (
        <>
            ...
            {mobileMenuOpen && (
                <div
                    className={cn(
                        'fixed inset-0 z-40 bg-black/40 backdrop-blur-xs md:hidden transition-opacity duration-200',
            ...
            {/* Mobile Drawer Panel (Slides out smoothly upwards from the capsule) */}
            {mobileMenuOpen && (
                <div
                    ...
                    className="fixed bottom-20 inset-x-4 max-w-sm sm:max-w-md mx-auto z-50 md:hidden pointer-events-none"
                >
                    <div
                        className={cn(
                            'pointer-events-auto w-full max-h-[calc(100dvh-7.5rem)] rounded-3xl bg-card/95 backdrop-blur-2xl border border-border/80 shadow-2xl ring-1 ring-foreground/10 p-5 flex flex-col gap-4 overflow-hidden',
                            isClosing ? 'animate-dock-slide-down' : 'animate-dock-slide-up'
                        )}
                    >
                        {/* Drawer Header Bar */}
