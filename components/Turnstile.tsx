    ...
    useEffect(() => {
        ...
        try {
            ...
            const id = window.turnstile.render(containerRef.current, {
                ...
                'timeout-callback': () => {
                    if (!isCancelled) {
                        setRenderError('Verification timed out. Please check your connection and retry.');
                ...
                'error-callback': (code) => {
                    if (!isCancelled) {
                        console.error(`[Turnstile] Widget error (sitekey ${siteKey.slice(0, 6)}...):`, code);
    ...
    return (
        <div className={`flex flex-col items-center justify-center my-3 ${className || ''}`}>
            ...
            {renderError && (
                <div
                    role="alert"
                    className="mt-2 w-full max-w-sm p-3 rounded-xl bg-destructive/10 border border-destructive/20 text-center"
                >
